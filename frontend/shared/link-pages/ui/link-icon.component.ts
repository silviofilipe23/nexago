...
import type { LinkIconName } from '../link-page.model';

/** Ícones stroke-24 do catálogo de links (`LinkIconName`). Um componente só, compartilhado
 *  pelo painel da arena e do organizador — a página pública redesenha os mesmos traços em React. */
@Component({
  selector: 'nx-link-icon',
  ...
  template: `
    ...
    >
      @switch (name()) {
        ...
        @case ('video') {
          <rect x="2.5" y="5" width="14" height="14" rx="3" /><path d="m16.5 10 5-3v10l-5-3z" />
        }
        @case ('globe') {
...
export class LinkIconComponent {
  readonly name = input.required<LinkIconName>();
