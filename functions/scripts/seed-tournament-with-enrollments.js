...
const TOURNAMENT_NAME =
  argValue("--tournament-name") || "Torneio seed nexaGO — 6 categorias";
...
function buildTournamentDoc(categories) {
  const now = new Date();
...
async function findTournamentByName(name) {
...
async function loadSeedAthletesByCategory() {
  ...
  for (const doc of snap.docs) {
    const data = doc.data();
    if (!isSeedAthlete(data, doc.id)) continue;
...
async function loadEnrolledUids(tournamentId) {
...
async function applyPaidPlans(tournamentId, tournament, plans) {
...
async function refreshTournamentStats(tournamentId, tournament) {
  const snap = await db
    .collection(artifactsInscriptionsPath(projectId))
    .where("tournamentId", "==", tournamentId)
    .get();
...
async function run() {
  ...
  let tournament = await findTournamentByName(TOURNAMENT_NAME);
  let tournamentId;
  ...
  if (tournament) {
    ...
    const doc = buildTournamentDoc(categories);
  ...
  const enrolledUids = await loadEnrolledUids(tournamentId);
  ...
  const pairs = await applyPaidPlans(tournamentId, tournament.data, plans);
  const stats = await refreshTournamentStats(tournamentId, tournament.data);
