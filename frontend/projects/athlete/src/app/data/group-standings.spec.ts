...
describe('buildGroupStandings', () => {
  it('lista todas as duplas sorteadas no grupo antes de qualquer resultado', () => {
    ...
    const rows = buildGroupStandings(matches, 'c', 'A');
  ...
  it('não cria linha para slot ainda sem dupla', () => {
    const rows = buildGroupStandings([matchOf({ poolId: 'A', teamAId: 't1', teamBId: '' })], 'c', 'A');
  ...
  it('ignora partidas de outros grupos', () => {
    ...
    expect(buildGroupStandings(matches, 'c', 'A').map((r) => r.teamId).sort()).toEqual(['t1', 't2']);
  });

  /** Todo torneio com fase de grupos tem um "Grupo A" em CADA categoria: os geradores numeram os
   *  grupos por categoria (`String.fromCharCode(65 + i)`). Filtrar só por `poolId` sobre as
   *  partidas do torneio inteiro fundia os dois — um grupo de 4 duplas aparecia com 8 no Modo
   *  Focus. */
  it('ignora o grupo de mesmo id de outra categoria', () => {
    const matches = [
      matchOf({ poolId: 'A', teamAId: 't1', teamBId: 't2' }),
      matchOf({ poolId: 'A', teamAId: 't3', teamBId: 't4' }),
      matchOf({ categoryId: 'outra', poolId: 'A', teamAId: 'x1', teamBId: 'x2' }),
      matchOf({ categoryId: 'outra', poolId: 'A', teamAId: 'x3', teamBId: 'x4' }),
    ];

    expect(buildGroupStandings(matches, 'c', 'A').map((r) => r.teamId).sort()).toEqual(['t1', 't2', 't3', 't4']);
    expect(buildGroupStandings(matches, 'outra', 'A').map((r) => r.teamId).sort()).toEqual(['x1', 'x2', 'x3', 'x4']);
  });
  ...
  it('conta vitória, derrota, sets e games só das partidas encerradas', () => {
    ...
    const [first] = buildGroupStandings(matches, 'c', 'A');
  ...
  it('registra a derrota de quem perdeu', () => {
    const rows = buildGroupStandings([completed('A', 't1', 't2', [{ a: 21, b: 15 }, { a: 21, b: 17 }], 't1')], 'c', 'A');
  ...
  it('aproveita o placar legado gravado em resultA/resultB', () => {
    ...
    const winner = buildGroupStandings([legacy], 'c', 'A').find((r) => r.teamId === 't1');
