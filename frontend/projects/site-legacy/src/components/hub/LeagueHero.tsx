...
/**
 * Cabeçalho cinematográfico da liga: capa full-bleed sob o header flutuante,
 ...
 */
export function LeagueHero({ league }: { league: LeagueSummary }) {
  ...
  return (
    ...
    >
      ...
      <div ref={imageRef} className="absolute inset-0 scale-[1.08] will-change-transform">
        {league.coverUrl ? (
          <Image src={league.coverUrl} alt="" fill priority sizes="100vw" className="object-cover" />
        ) : (
          <div
            aria-hidden="true"
      ...
      <div ref={overlayRef} className="absolute inset-x-0 bottom-0 will-change-transform">
        <div className="mx-auto max-w-4xl px-5 pb-8 sm:px-6 sm:pb-10">
          ...
          <div className="mt-5 flex flex-col gap-3 text-text-mute sm:flex-row sm:flex-wrap sm:gap-6">
            ...
            {stageCount > 0 && (
              <span className="flex items-center gap-2">
                <Layers className="size-4 text-text-dim" aria-hidden="true" />
