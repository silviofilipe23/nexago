...
- **O que conta como eliminação** (emenda de 19/08, ver D4 da spec): partida cujo perdedor NÃO tem `loserAdvance`. Contar rodada não funciona — nas plantas de 4/5/6 a disputa de 3º puxa participantes de rodadas diferentes. Exceções que continuam por rodada: DE legada sem disputa de 3º (`maxLbRound` e `maxLbRound - 1` são pódio) e mata-mata simples (`round >= semifinalRound`, com `semifinalRound = finalRound > 1 ? finalRound - 1 : 1`). Chave sem `winnerAdvance`/`loserAdvance` nenhum: mapas vazios, nada de adivinhação.
