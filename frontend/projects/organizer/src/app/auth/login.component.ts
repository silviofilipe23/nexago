  ...
  template: `
    <og-auth-shell>
      <form [formGroup]="form" (ngSubmit)="submit()">
        ...
        <button class="og-btn-primary" type="submit" [disabled]="busy()">
          @if (loading()) {
        ...
        <div class="og-divider"><span>ou</span></div>

        <button class="og-btn-social" type="button" [disabled]="busy()" (click)="signInWithGoogle()">
          @if (googleLoading()) {
            <span class="og-spinner og-spinner-light" aria-hidden="true"></span>
            Entrando…
          } @else {
...
export class LoginComponent {
  ...
  /** Desabilita os dois botões enquanto qualquer um dos logins está em voo. */
  protected readonly busy = computed(() => this.loading() || this.googleLoading());
  ...
  /** Não passa pela validação do formulário: o login com Google dispensa
   *  e-mail e senha, e `submit()` abortaria com os campos vazios. */
  protected async signInWithGoogle(): Promise<void> {
    this.error.set(null);
    this.googleLoading.set(true);
    try {
      await this.auth.signInWithGoogle(this.form.getRawValue().remember);
      this.redirectAfterLogin();
    } catch (err) {
      this.error.set(mapFirebaseAuthError(err));
    } finally {
