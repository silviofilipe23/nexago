      ...
      @switch (state()) {
        ...
        @case ('invalid') {
          <div class="co-center">
            <header class="co-form-header">
              <span class="co-kicker">Redefinir senha</span>
              <h1>Link inválido ou expirado.</h1>
              <p>{{ invalidReason() }}</p>
            </header>
            <a class="co-btn-primary" routerLink="/entrar/recuperar">
              Pedir um novo link
            </a>
          </div>
        }
        @case ('ready') {
          <form [formGroup]="form" (ngSubmit)="submit()">
            ...
            <p class="co-fine">Isso desconecta as outras sessões ativas do painel.</p>
          </form>
        }
      }
    </co-auth-shell>
