/* eslint-disable */
/**
 * Remove usuários cujo nome contém "QA" (ex.: "Atleta QA 01", "QA Teste")
 * E que tenham papel de atleta (`roles` inclui "athlete", `role == "athlete"`
 * ou `hasAthleteRole == true`).
 *
 * Critério de nome (case-insensitive): needle aparece em
 *   - Auth: displayName
 *   - Firestore users/{uid}: fullName | name | nickname | displayName
 *
 * Critério de papel: só remove se for atleta (doc users e/ou claims do Auth).
 *
 * Com --yes:
 *   1) apaga users/{uid} (recursivo: subcoleções)
 *   2) apaga athlete_profiles/{uid} e public_profiles/{uid} se existirem
 *   3) apaga a conta no Firebase Auth (se existir)
 *
 ...
 *   node scripts/delete-users-qa-name.js --project volley-track-dev-4596c
 *   node scripts/delete-users-qa-name.js --project <projectId> --yes
 *   node scripts/delete-users-qa-name.js --project <projectId> --yes --limit 20
 *   node scripts/delete-users-qa-name.js --project <id> --needle QA
 *
...
const NAME_NEEDLE = (argValue("--needle") || "wes").toLowerCase();
...
function nameContainsNeedle(raw) {
  return typeof raw === "string" && raw.toLowerCase().includes(NAME_NEEDLE);
}
...
/** Doc Firestore ou claims: atleta se roles[] tem athlete, role legado, ou flag. */
...
/**
 * @typedef {{ uid: string, label: string, email: string, source: string }} Match
 */
...
async function collectFromAuth(byUid) {
  ...
  do {
    ...
    for (const user of page.users) {
      if (!nameContainsNeedle(user.displayName || "")) continue;
      if (!isAthleteFromClaims(user.customClaims)) {
        // Claims sem athlete: ainda pode ser atleta só no Firestore — marca e
        // deixa collectFromUsers / resolveAthlete confirmar.
        const existing = byUid.get(user.uid);
        byUid.set(user.uid, {
          uid: user.uid,
          label: pickLabel(user.displayName, existing?.label),
          email: user.email || existing?.email || "",
          source: existing ? `${existing.source}+auth-pending` : "auth-pending",
          _athleteConfirmed: existing?._athleteConfirmed === true,
        });
        continue;
      }
      const existing = byUid.get(user.uid);
      byUid.set(user.uid, {
        ...
        label: pickLabel(user.displayName, existing?.label),
        email: user.email || existing?.email || "",
        source: existing ? `${existing.source}+auth` : "auth",
...
async function collectFromUsers(byUid) {
  ...
  while (LIMIT == null || countConfirmed(byUid) < LIMIT) {
    ...
    for (const doc of snap.docs) {
      const data = doc.data();
      const names = [data.fullName, data.name, data.nickname, data.displayName];
      if (!names.some(nameContainsNeedle)) continue;
      if (!isAthleteFromData(data)) continue;

      const existing = byUid.get(doc.id);
      byUid.set(doc.id, {
        uid: doc.id,
        label: pickLabel(...names, existing?.label),
        email:
...
/** Confirma atleta nos pending do Auth via leitura do doc users. */
async function confirmPendingAthletes(byUid) {
  for (const [uid, match] of [...byUid.entries()]) {
    ...
    if (snap.exists && isAthleteFromData(snap.data())) {
      byUid.set(uid, {
        ...match,
        source: `${match.source.replace("-pending", "")}+users`,
        _athleteConfirmed: true,
        label: pickLabel(
          snap.data().fullName,
          snap.data().name,
        ...
        email:
          (typeof snap.data().email === "string" && snap.data().email.trim()) ||
          match.email,
      });
    } else {
      byUid.delete(uid);
...
async function run() {
  ...
  console.log(
    `Filtro: nome contém "${NAME_NEEDLE}" E role athlete${LIMIT ? ` (limit=${LIMIT})` : ""}`,
  );
  ...
  console.log(`Encontrados ${limited.length} atleta(s) com nome contendo "${NAME_NEEDLE}".`);
  if (limited.length === 0) {
  ...
  for (const m of preview) {
    console.log(`  - ${m.label} | ${m.email || "sem e-mail"} | ${m.uid} [${m.source}]`);
  }
  ...
  for (const m of limited) {
    try {
      ...
      firestoreFail += 1;
      console.warn(`Firestore falhou para ${m.label} (${m.uid}):`, err.message || err);
    }
  }
