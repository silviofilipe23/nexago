      ...
      @switch (state()) {
        ...
        @case ('invalid') {
          <div class="ar-center">
            <header class="ar-form-header">
              <span class="ar-kicker">Redefinir senha</span>
              <h1>Link inválido ou expirado.</h1>
              <p>{{ invalidReason() }}</p>
            </header>
            <a class="ar-btn-primary" routerLink="/entrar/recuperar">
              Pedir um novo link
            </a>
          </div>
        }
        @case ('ready') {
          <form [formGroup]="form" (ngSubmit)="submit()">
            ...
            <p class="ar-fine">Isso desconecta as outras sessões ativas do painel.</p>
          </form>
        }
      }
    </ar-auth-shell>
