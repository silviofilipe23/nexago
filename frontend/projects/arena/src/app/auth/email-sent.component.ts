  ...
  template: `
    <ar-auth-shell>
      <div class="ar-center">
        ...
        <a class="ar-btn-primary" href="https://mail.google.com" target="_blank" rel="noopener">
          Abrir e-mail
        </a>

        <div class="ar-resend-row">
          Não chegou? Confira o spam ou
          <button class="ar-text-link" type="button" [disabled]="cooldown() > 0" (click)="resend()">reenviar</button>
          @if (cooldown() > 0) {
            <span class="ar-timer">({{ cooldownLabel() }})</span>
          }
        </div>
      </div>
    </ar-auth-shell>
