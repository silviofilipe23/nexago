...
const inter = (weight: number, size: number) => `${weight} ${size}px "Inter", system-ui, sans-serif`;
 ...
 *  compartilhamento falha. Foto que não permite CORS resolve `null` e o círculo cai
 *  nas iniciais — nunca quebra a arte. */
...
function tracked(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, spacing: number, align: 'left' | 'center' = 'center'): void {
...
/** Reduz a fonte até o texto caber. Deixa ctx.font ajustada e retorna o tamanho. */
function fitFont(ctx: CanvasRenderingContext2D, text: string, maxWidth: number, start: number, min: number, font: (size: number) => string): number {
  let size = start;
  ctx.font = font(size);
...
function drawMatchup(ctx: CanvasRenderingContext2D, data: ShareCardData, photos: Map<string, HTMLImageElement | null>, m: Metal): void {
  ...
  const drawTeam = (team: ShareTeam, photoCy: number, nameY: number) => {
    ...
    fitFont(ctx, team.name, 930, 84, 52, (s) => sora(800, s));
    ctx.fillStyle = INK;
    ctx.textAlign = 'center';
...
export async function drawShareCard(ctx: CanvasRenderingContext2D, data: ShareCardData): Promise<void> {
  const fontSpecs = [sora(800, 210), sora(800, 96), sora(800, 64), sora(800, 46), mono(800, 215), mono(700, 36), mono(500, 26), inter(600, 40)];
