import { bracketGroupKey, bracketGroupSortOrder, buildBracketColumns, isBracketMatch, type TournamentMatch } from '../data/matches-repository';
...
/** Largura/altura do card — precisam bater exatamente com `.bx-match` em
 *  `category-bracket.component.scss`, senão os conectores desalinham. (app: 280×150 numa tela
 *  dedicada) Altura = head 28 + 2 lados de 40 + rodapé de agendamento 28 (dia · hora · quadra)
 *  = 136 de CONTEÚDO — mas o card renderiza 138: soma a borda de 1px do próprio `.bx-match`
 *  (topo e base). Aqui a altura é explícita (`[style.height.px]="matchHeight"`, no template) e
 *  `box-sizing: border-box` é global (Tailwind preflight), então a borda entra DENTRO dos 138 —
 *  sobra exatamente 136 de conteúdo pros 4 filhos empilhados, sem cortar nada. Mesma conta do
 *  organizador (`bracket-tree.ts` do painel), por um caminho CSS diferente: lá o card não
 *  declara altura própria e cresce pro conteúdo, com a borda somando por FORA dos 136 — os dois
 *  chegam em 138. Usar 136 aqui cortava 2px do card (rodapé de agendamento espremido) e
 *  desalinhava todo conector 1px acima do centro visual. */
...
function bracketColumnHeaderLabel(columnMatches: readonly TournamentMatch[]): string {
  ...
  if (type === 'wb' || type === 'lb') return `${type.toUpperCase()} · RODADA ${first.round}`;
  if (isFinalType(type)) return 'FINAL';
  if (isThirdPlaceType(type)) return '3º LUGAR';
  return (first.matchType.trim() || `RODADA ${first.round}`).toUpperCase();
}
...
export function buildDoubleEliminationLayout(matches: readonly TournamentMatch[]): BracketLayout | null {
  ...
  for (const col of columnKeys) {
    ...
    for (const match of columnMatches) {
      ...
      const node: BracketLayoutNode = { match, left: x, top: centerY - BRACKET_MATCH_HEIGHT / 2 };
      nodes.push(node);
 ...
 *  `debugEndFor` no app). */
function pathFor(from: BracketLayoutNode, to: BracketLayoutNode): string {
 ...
 *  quem alimentou quem. */
function buildAdvanceEdges(matches: readonly TournamentMatch[], nodeByMatchNumber: Map<number, BracketLayoutNode>): BracketLayoutEdge[] {
  const byNumber = new Map<number, TournamentMatch>();
  for (const m of matches) byNumber.set(m.matchNumber, m);
...
export function buildKnockoutTreeLayout(matches: readonly TournamentMatch[]): BracketLayout | null {
  ...
  const drawEdge = (from: BracketLayoutNode, to: BracketLayoutNode): void => {
    edges.push({ d: pathFor(from, to) });
  };
