      ...
      @switch (state()) {
        ...
        @case ('invalid') {
          <div class="bo-center">
            <header class="bo-form-header">
              <span class="bo-kicker">Redefinir senha</span>
              <h1>Link inválido ou expirado.</h1>
              <p>{{ invalidReason() }}</p>
            </header>
            <a class="bo-btn-primary" routerLink="/entrar/recuperar" style="text-decoration: none;">
              Pedir um novo link
            </a>
          </div>
        }
        @case ('ready') {
          <form [formGroup]="form" (ngSubmit)="submit()">
            ...
            <p class="bo-fine">Isso desconecta tuas outras sessões ativas.</p>
          </form>
        }
      }
    </bo-auth-shell>
...
export class ResetPasswordComponent implements OnInit {
  ...
  protected readonly invalidReason = signal('O link de redefinição não é mais válido. Pede um novo que a gente reenvia.');
  ...
  protected async submit(): Promise<void> {
    ...
    try {
      ...
      await this.auth.signInWithEmail(this.email(), password);
